...
class TagAligner:
    ...
    def approach(self, expected_id, distance):
        if distance <= FINAL_APPROACH_M:
            raise ValueError("Approach must include a tracked segment before the final 12 cm")
        ...
        while travelled < tracked_distance - 1e-8:
            ...
            travelled += step
        # Finish from the 48 cm point without requiring the tag near the claw.
        print(f"Tag {expected_id} approach aligned; driving final 12 cm without vision...", flush=True)
